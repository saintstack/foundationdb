...
ACTOR static Future<std::pair<GetKeyValuesReply, GetKeyValuesReply>> fetchSourceAndRestoredData(StorageServer* data,
	...
	try {
		...
		state RangeResult sourceData = wait(tr.getRange(rangeToRead, sourceLimits, Snapshot::False, Reverse::False));
...
ACTOR Future<Void> auditRestoreQ(StorageServer* data, AuditStorageRequest req) {
	...
	state Key rangeToReadBegin = req.range.begin;
	state KeyRange claimRange;
	state int limit = SERVER_KNOBS->AUDIT_RESTORE_BATCH_KEY_LIMIT; // Use knob instead of hardcoded 10K
	state int limitBytes = CLIENT_KNOBS->REPLY_BYTE_LIMIT;
	state int64_t readBytes = 0;
	state Error nonRetryableError;
	state int64_t numValidatedKeys = 0;
	state int64_t validatedBytes = 0;
	state int64_t lastPersistBytes = 0; // Track when we last persisted progress
	...
	try {
		loop {
			try {
				...
				nonRetryableError = e;
				// Transient errors — the SS serving restored data may be overloaded
				// or not caught up yet after a restore. Retry with a fresh version.

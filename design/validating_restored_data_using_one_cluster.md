...
* Restore:
    * Setup: Enable a new knob **restore_validation**.  The setup should bypass the restore to empty cluster check
    * Verify restored_data_prefix keyRange **** does not have data → GetMaxRestoreVersion from backup → Start restore with add-prefix option → Wait for restore completion
    * Note: 
...
* **restore_validation knob:** [EmptyCluserCheck](https://github.com/apple/foundationdb/blob/release-7.4/fdbclient/FileBackupAgent.actor.cpp#L7081) This check need to be bypassed if this knob is set. Also also don’t unlock the database in restore code if this knob is set.
* **Audit Storage:**

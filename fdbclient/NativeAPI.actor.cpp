...
// CROSS_PROCESS_FIX: Database constructor implementations
Database::Database(Reference<DatabaseContext> cx) : db(cx) {
	// CROSS_PROCESS_FIX: Initialize creatingProcess when Database is created
	if (g_network && g_network->isSimulated() && db && !db->creatingProcess.isValid()) {
		db->creatingProcess = g_network->getLocalAddress();
	}
}

Database::Database(DatabaseContext* cx) : db(cx) {
	// CROSS_PROCESS_FIX: Initialize creatingProcess when Database is created
	if (g_network && g_network->isSimulated() && db && !db->creatingProcess.isValid()) {
		db->creatingProcess = g_network->getLocalAddress();
	}
...
// CROSS_PROCESS_FIX: Database copy constructor with cross-process cloning
Database::Database(Database const& rhs) : db(rhs.db) {
	// CROSS_PROCESS_FIX: Simple sharing approach - S3 connection pool isolation handles cross-process safety
	if (false && g_network && g_network->isSimulated() && rhs.db && 
	    rhs.db->creatingProcess.isValid() && 
	    rhs.db->creatingProcess != g_network->getLocalAddress()) {
		
		// Cross-process copy - check cache first to avoid excessive cloning
		static std::map<std::pair<NetworkAddress, DatabaseContext*>, Reference<DatabaseContext>> cloneCache;
		
		NetworkAddress currentProcess = g_network->getLocalAddress();
		auto cacheKey = std::make_pair(currentProcess, rhs.db.getPtr());
		
		auto it = cloneCache.find(cacheKey);
		if (it != cloneCache.end()) {
			// Reuse cached clone
			db = it->second;
			// DISABLED: This logging was causing trace line overflow
			// printf("T=%.6f DB_CROSS_PROCESS_COPY_CACHED: original_ctx=%p cached_ctx=%p process=%s\n",
			//        g_network->now(),
			//        rhs.db.getPtr(),
			//        db.getPtr(),
			...
			// Create new clone and cache it
			// DISABLED: This logging was causing DeliverAttempt trace overflow
			// printf("T=%.6f DB_CROSS_PROCESS_COPY_NEW: original_ctx=%p creator=%s current=%s - CLONING\n",
			//        g_network->now(),
			//        rhs.db.getPtr(),
			//        rhs.db->creatingProcess.toString().c_str(),
			//        currentProcess.toString().c_str());
			// fflush(stdout);
			
			db = rhs.db->clone().db;  // Use clone() to get new DatabaseContext
			cloneCache[cacheKey] = db;  // Cache the clone
			
			// DISABLED: Keep only the essential CLONING message above
			// printf("T=%.6f DB_CROSS_PROCESS_COPY_NEW: new_ctx=%p process=%s\n",
			//        g_network->now(),
			//        db.getPtr(),
			//        currentProcess.toString().c_str());
			// fflush(stdout);
		...
		// Same process copy or uninitialized creatingProcess - sharing is fine
		// DISABLED: This logging was causing trace line overflow
		// if (g_network && g_network->isSimulated()) {
		// 	printf("T=%.6f DB_SAME_PROCESS_COPY: ctx=%p process=%s task=%d\n",
		// 	       g_network->now(),
		// 	       db.getPtr(),
		// 	       g_network->getLocalAddress().toString().c_str(),
		// 	       (int)g_network->getCurrentTask());
		// 	fflush(stdout);
		// }
		
		// Initialize creatingProcess for objects that don't have it set
		if (g_network && g_network->isSimulated() && db && !db->creatingProcess.isValid()) {
			db->creatingProcess = g_network->getLocalAddress();
		}
	}
...
// CROSS_PROCESS_FIX: Database assignment operator with cross-process cloning
void Database::operator=(Database const& rhs) {
	// CROSS_PROCESS_FIX: Simple sharing approach - let NetSAV protection handle cross-process safety  
	db = rhs.db;
	if (false && g_network && g_network->isSimulated() && rhs.db && 
	    rhs.db->creatingProcess.isValid() && 
	    rhs.db->creatingProcess != g_network->getLocalAddress()) {
		
		// Cross-process assignment - check cache first to avoid excessive cloning
		static std::map<std::pair<NetworkAddress, DatabaseContext*>, Reference<DatabaseContext>> cloneCache;
		...
		if (it != cloneCache.end()) {
			...
			db = it->second;
			// DISABLED: This logging was causing DeliverAttempt trace overflow
			// printf("T=%.6f DB_CROSS_PROCESS_ASSIGN_CACHED: original_ctx=%p cached_ctx=%p process=%s\n",
			//        g_network->now(),
			//        rhs.db.getPtr(),
			//        db.getPtr(),
			//        currentProcess.toString().c_str());
			...
			// Create new clone and cache it
			// DISABLED: This logging was causing DeliverAttempt trace overflow
			// printf("T=%.6f DB_CROSS_PROCESS_ASSIGN_NEW: original_ctx=%p creator=%s current=%s - CLONING\n",
			//        g_network->now(),
			//        rhs.db.getPtr(),
			//        rhs.db->creatingProcess.toString().c_str(),
			//        currentProcess.toString().c_str());
		...
		// Same process assignment - sharing is fine
		db = rhs.db;

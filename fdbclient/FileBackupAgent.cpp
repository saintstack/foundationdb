...
Future<bool> monitorBulkLoadJobCompletionWithProgress(Database cx,
	...
	while (true) {
		...
		if (!stillRunning) {
			// Job completed - set final progress (best-effort, don't fail the restore)
			try {
				auto [completed, submitted, triggered, running, total, bytes] =
				    co_await getBulkLoadTaskProgress(cx, jobId, lockAware);
				int64_t finalBlocks = totalBlocks > 0 ? totalBlocks : total;
				...
				restore.fileBlocksFinished().set(tr, finalBlocks);
				restore.filesBlocksDispatched().set(tr, finalBlocks);
				restore.bytesWritten().set(tr, bytes);
				co_await tr->commit();
			} catch (Error& e) {
				TraceEvent(SevWarn, "BulkLoadRestoreFinalProgressError").error(e).detail("JobId", jobId);
			}
			co_return true;
		}

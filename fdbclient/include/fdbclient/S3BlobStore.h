...
public:
	...
	void maybeStartStatsLogger() {
		if (!blobStats && CLIENT_KNOBS->BLOBSTORE_ENABLE_LOGGING) {
			...
			specialCounter(
			    blobStats->cc, "GlobalConnectionPoolCount", [this]() { return this->getGlobalConnectionPool().size(); });
	...
	struct ReusableConnection {
		...
		ReusableConnection(const ReusableConnection& other) : conn(other.conn), expirationTime(other.expirationTime), creatingProcess(other.creatingProcess) {
			if (g_network && g_network->isSimulated() && 
			    creatingProcess.isValid() && 
			    creatingProcess != g_network->getLocalAddress()) {
	...
	static std::unordered_map<BlobStoreConnectionPoolKey, Reference<ConnectionPoolData>>& getGlobalConnectionPool() {
		...
		static std::map<NetworkAddress, std::unordered_map<BlobStoreConnectionPoolKey, Reference<ConnectionPoolData>>> processConnectionPools;
